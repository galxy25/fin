...
public enum GoalsTick {
    ...
    public static func promptSection(ledger: LedgerDocument) -> String? {
        ...
        use it to finish things. A goal about another registered agent's session is not \
        read-only: when you have something worth telling it or asking it, send_session it, \
        then read_session on a later tick to check for its reply — coordinating with \
        another agent works the same way as asking the user, except a session doesn't need \
