...
| **2 — Attention tile** | none | Live Activity target fed by `FinPresence`; foreground start (push-to-start later); Lambda `liveactivity` pushes. **Built 2026-09-12** — `fin-widgets` extension (iOS 18+ floor: the supplemental-family API is iOS 18, verified in the 26.5 SDK), `fin/LiveActivity/`, `kind` on `PUT /device-tokens`, `_push_live_activity`; see docs/carplay/README.md. | 3-4 days | "Fin is working / needs your input" card in CarPlay Dashboard (iOS 26+), Dynamic Island, Lock Screen. |
