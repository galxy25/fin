#!/usr/bin/env bash
#
# testflight-visionos.sh — archive Fin (native visionOS) and upload the build to TestFlight.
#
# Sibling of testflight.sh (iOS) and testflight-macos.sh. Same local-archive +
# ASC-API-key flow, but builds the NATIVE visionOS app (Apple Vision Pro), not the
# "Designed for iPad" compatibility variant.
#
# The app's visionOS platform in App Store Connect is created automatically on the
# first successful visionOS upload — the iOS app record (dev.levischoen.fin) already
# exists, no separate "New App" step needed for this platform (same as macOS was).
#
...
#
# USAGE:
#   scripts/testflight-visionos.sh            # build number = current unix timestamp
#   BUILD_NUMBER=42 scripts/testflight-visionos.sh
#
set -euo pipefail
...
TEAM_ID="EC27UF79GL"
SCHEME="fin"
ARCHIVE="build-release/fin-visionOS.xcarchive"
EXPORT_DIR="build-release/export-visionos"
...
# cert + visionOS App Store provisioning profile (the key must be Admin role), so no
# Apple ID needs to be signed into Xcode (headless).
xcodebuild \
...
EXPORT_PLIST="$(mktemp -t FinVisionExportOptions).plist"
...
echo "==> Exporting + uploading to TestFlight (visionOS)"
...
echo "==> Done. visionOS build $BUILD_NUMBER uploaded; it'll appear under the app's visionOS"
