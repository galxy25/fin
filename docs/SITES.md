...
**The shared tmux socket, and what actually guards it.** `connectCommand` puts the daemon's shell on the *default* tmux socket — the same server as the human's own sessions — so registration alone was never a gate. `TmuxCommandGuard` (`daemon/Sources/FinAgentCore/TmuxCommandGuard.swift`, wired into `AgentTurnEngine.executeSendInput`) parses every `send_input` string and refuses mutating tmux commands whose target is not the daemon's own session, a `routing-registry.json` entry read at launch, or a session in Fin's own `fin-` namespace, while leaving all inspection (`capture-pane`, `list-*`, `has-session`, `display-message -p`) unrestricted against every session — the read half is what makes a resident site worth having. Because tmux never sees shell quoting, the parser reasons about the argv tmux gets: a quoted `';'` separates commands, `-lt main` hides a target inside a getopt cluster, and `kill-session -a -t fin` kills every session *except* `fin` — all three were verified against tmux 3.6a on a private socket and are refused. The registry is a launch snapshot on purpose: it is a file the guarded shell can write, so a live re-read would let the model widen its own allow-list — but the snapshot is a speed bump rather than a barrier, because the LaunchAgent is `KeepAlive` and the agent can end the process to force a re-read of a file it just wrote; closing that needs an allow-list this uid cannot write, or the dedicated socket. An unrecognized head is no longer a way past the parser (a bare `tmux` token is checked under `find -exec`, shell keywords, and a pipe into `sh`), tmux's own global flags are parsed as getopt clusters (`tmux -2f ls kill-session -t victim` really did kill it), and this machine's own hostname and addresses count as local for `ssh`. It is still defense in depth over a natural-language channel, not isolation: variable/alias indirection, `eval`/base64, a helper script, an argv assembled structurally inside an interpreter, or a command split across two sends all remain open, as does non-tmux damage; a remote `ssh … tmux` is out of scope by policy, since another machine's session names are not in this registry's namespace. Isolation is `connectCommand: "tmux -L fin new-session -A -s fin"`, which deletes the human's sessions from the daemon's namespace and, with them, Fin's ability to read the machine's real work. See `daemon/README.md` § "The tmux send-keys guard" for the policy table and the full residual list.
