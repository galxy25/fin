...
**The shared tmux socket, and what actually guards it.** `connectCommand` puts the daemon's shell on the *default* tmux socket — the same server as the human's own sessions — so registration alone was never a gate. `TmuxCommandGuard` (`daemon/Sources/FinAgentCore/TmuxCommandGuard.swift`, wired into `AgentTurnEngine.executeSendInput`) parses every `send_input` string and refuses mutating tmux commands whose target is not the daemon's own session or a `routing-registry.json` entry, while leaving all inspection (`capture-pane`, `list-*`, `has-session`, `display-message -p`) unrestricted against every session — the read half is what makes a resident site worth having. It is defense in depth over a natural-language channel, not isolation: variable/alias indirection, `eval`/base64, a helper script, or a remote `ssh … tmux` all remain open, as does non-tmux damage. Isolation is `connectCommand: "tmux -L fin new-session -A -s fin"`, which deletes the human's sessions from the daemon's namespace and, with them, Fin's ability to read the machine's real work. See `daemon/README.md` § "The tmux send-keys guard" for the policy table and the full residual list.
