...
and appends `restrict,pty,from="127.0.0.1,::1" ssh-ed25519 … fin-site-<siteId8>` to `~/.ssh/authorized_keys`. `restrict` disables forwarding and X11, `pty` re-enables the terminal the harness needs, `from=` pins it to loopback. Distinct comments (`fins-key` vs `fin-site-*`) make either revocation a one-liner. `LC_FIN_AGENT` still crosses (`AcceptEnv LANG LC_*` is the macOS default), so the fish auto-attach guard keeps the daemon out of Levi's own tmux. The site uses `connectCommand: "exec tmux -L fin new-session -A -s fin \; set status off"` (the tested path since 2026-09-06): `-L fin` puts the daemon's shell on its **own tmux server**, a different socket file and a different process from the one hosting Levi's `main`, and `exec` means leaving that tmux ends the SSH session rather than dropping the agent back into an unconfined login shell (the daemon reconnects, with backoff). A forced-command variant, and a dedicated UNIX user, are the Phase 2 hardening options.

**The private tmux socket, and what still guards the edges.** `connectCommand` now carries `exec tmux -L fin …`, so the daemon's shell talks to a tmux server that is a different process, with a different socket file, from the one hosting Levi's sessions — and `exec` means leaving that tmux ends the SSH session instead of dropping the agent into the login shell underneath, where `$TMUX` is empty. That is the boundary, and it is topological: no string the model types inside that shell reaches `main`, because the server it would have to talk to is not the one its `$TMUX` points at. It replaces the previous posture — one shared socket plus a 1,579-line parser classifying every `send_input` string — which eight independent reviews walked through **thirty-five distinct high-severity bypasses**, each fix round producing new ones. The cost is the read half, and that is restored structurally rather than given up: the new `read_session` tool takes a session **name** (`^[A-Za-z0-9_.:-]{1,64}$`, no leading `-`) and the daemon runs a fixed `tmux capture-pane -p -J -t <name> -S -<lines>` against the DEFAULT socket over a separate SSH exec channel — the model supplies one word, the daemon supplies every other byte, and the output is capped and redacted. `TmuxCommandGuard` traded a 90-command policy table for seven rules, and — as of 2026-09-06 — stopped asking which ENVIRONMENT a command would run in at all: **every tmux command must name Fin's own server** (`tmux -L fin …`, with `-S` beating `-L` the way tmux itself resolves them), so naming another socket is refused and so is naming none. That one rule replaced two mechanisms that were guessing where a socket-less `tmux …` would land — a live `$TMUX` probe typed into the PTY before every tmux-bearing send, and a named list of environment-crossing wrappers (`ssh`, `sudo`, `env -i`, `launchctl`…) — neither of which was complete, and the probe of which was answered by the very terminal being checked (a `sed` left running in the pane can forge it). `-L fin` reaches Fin's server from any shell, confined or not, so the question is deleted rather than answered. The rest: refuse `kill-server` and its prefixes, refuse any signal aimed at tmux (`pkill tmux`, `kill $(pgrep tmux)` — a signal crosses every socket), refuse any command that unsets or overrides `TMUX` or `TMUX_TMPDIR`, refuse a half-typed line that mentions tmux (the PTY joins consecutive sends), refuse a line that names tmux and starts a command with a substitution (`$(which tmux) -L default …`, where the program is decided at runtime), and refuse a line that hands tmux text to a scheduler (`crontab -`, `at`, `launchctl`), which would run it later in a shell nobody is judging. Everything else on Fin's own server is ordinary work — no allow-list, nothing to register. **Residual, plainly:** `TMUX_TMPDIR` is the directory a `-L <label>` resolves in (verified on 3.6a against private sockets only), so an *assembled* edit — a variable set in one send and used in the next, a helper script, an argv built inside an interpreter — re-points the very flag the rule demands; the "name your server" rule fires where the guard can see tmux in command position, so a socket-less tmux under an unmodelled head (`find . -exec tmux send-keys …`) is allowed, because that shape is far more often prose (`man tmux`, `grep -e tmux config.fish`); the shell runs as Levi's own uid, so the default socket is reachable by any process that opens it deliberately — including by replacing Fin's own socket file with a symlink to it, which no parser can see; the launch-time `$TMUX` probe is a log line only, and an unconfined shell now costs durability (work outside a tmux session dies with the connection) rather than safety; and non-tmux damage was never in scope. The airtight version is a **dedicated UNIX user**, whose different uid cannot open a `0700` socket directory by kernel permission — one `sudo` step, currently out of scope because the installer is deliberately zero-sudo, and Levi's call to make. See `daemon/README.md` § "The tmux boundary" for the rule table and the full residual list.
