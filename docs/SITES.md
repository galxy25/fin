...
and appends `restrict,pty,from="127.0.0.1,::1" ssh-ed25519 … fin-site-<siteId8>` to `~/.ssh/authorized_keys`. `restrict` disables forwarding and X11, `pty` re-enables the terminal the harness needs, `from=` pins it to loopback. Distinct comments (`fins-key` vs `fin-site-*`) make either revocation a one-liner. `LC_FIN_AGENT` still crosses (`AcceptEnv LANG LC_*` is the macOS default), so the fish auto-attach guard keeps the daemon out of Levi's own tmux. The site uses `connectCommand: "tmux -L fin new-session -A -s fin \; set status off"` (the tested path since 2026-09-06): `-L fin` puts the daemon's shell on its **own tmux server**, a different socket file and a different process from the one hosting Levi's `main`. A forced-command variant, and a dedicated UNIX user, are the Phase 2 hardening options.

**The private tmux socket, and what still guards the edges.** `connectCommand` now carries `-L fin`, so the daemon's shell talks to a tmux server that is a different process, with a different socket file, from the one hosting Levi's sessions. That is the boundary, and it is topological: no string the model types inside that shell reaches `main`, because the server it would have to talk to is not the one its `$TMUX` points at. It replaces the previous posture — one shared socket plus a 1,579-line parser classifying every `send_input` string — which eight independent reviews walked through **thirty-five distinct high-severity bypasses**, each fix round producing new ones. The cost is the read half, and that is restored structurally rather than given up: the new `read_session` tool takes a session **name** (`^[A-Za-z0-9_.:-]{1,64}$`, no leading `-`) and the daemon runs a fixed `tmux capture-pane -p -J -t <name> -S -<lines>` against the DEFAULT socket over a separate SSH exec channel — the model supplies one word, the daemon supplies every other byte, and the output is capped and redacted. `TmuxCommandGuard` shrank from 1,579 lines to 1,078 (670 of them code, down from 1,057) and from a 90-command policy table to six rules: refuse EVERY tmux command when the daemon could not confirm the shell is inside its own tmux server (it asks the live shell for `$TMUX` after connecting — a quietly-failed `connectCommand` would otherwise leave bare tmux commands landing on the default socket), refuse a tmux invocation naming another socket (`-L`/`-S`), refuse `kill-server` and its prefixes, refuse `pkill`/`killall tmux` (a signal crosses every socket), refuse any command that unsets or overrides `TMUX`, and refuse a half-typed line that mentions tmux (the PTY joins consecutive sends). Everything else on Fin's own server is ordinary work — no allow-list, nothing to register. **Residual, plainly:** tmux reads its socket path from `$TMUX` when no `-L`/`-S` is given (verified on 3.6a against two private sockets: from a pane on socket A, `TMUX=<socket B>,0,0 tmux ls` listed B's sessions), so an *assembled* unset — a variable set in one send and used in the next, a helper script, an argv built inside an interpreter — still reaches the default socket; the shell runs as Levi's own uid, so that socket is reachable by any process that opens it deliberately; and non-tmux damage was never in scope. The airtight version is a **dedicated UNIX user**, whose different uid cannot open a `0700` socket directory by kernel permission — one `sudo` step, currently out of scope because the installer is deliberately zero-sudo, and Levi's call to make. See `daemon/README.md` § "The tmux boundary" for the rule table and the full residual list.
