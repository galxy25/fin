...
3. Check the qualifiers are *in the sentence*, not merely in the surrounding
   ...
   **re-check**, which is a decision a reviewer can then check by hand; or, for
   ...
   `Negated number (§4.3): yes`. A row that declares the carve-out may not have
   its sentence appear in a heading or in the piece's `title:` — that is the
   "never the most quotable sentence" half, and it is the reason the
   declaration cannot simply be typed to get past the check. This is a shape
   test, not a reading: it cannot tell whether the model named is the model
   that was run. It exists because `RPI-12` shipped a bare `36/51` past a green
   checker (§8), and it would have failed that row — **but only in the form it
   has now.** For a day it matched the word "carve-out" in any polarity, so
   `RPI-12`'s own corrected re-check ("No carve-out (§4.3), and none is
   available here") exempted the row the check had been written for. That is
   the fifth entry in §8, and the reason the shapes are spelled out above.
...
now failed its own rules five times: twice outright, once by searching too
narrowly, once by counting an artifact that was still being written, and once
by describing a check as tighter than the check was. All five are recorded here
rather than quietly edited, for the reason `README.md` §1 gives for the internal
record: a correction nobody can see is indistinguishable from a claim nobody
checked. A ledger that logs its own failures is worth more than one that appears
...
here", written the same day to fix `RPI-12` — bought `RPI-12` the exemption that
sentence exists to refuse. A correction that silently disables the check written
to enforce it is worse than no correction, and nothing in a green run says so.
