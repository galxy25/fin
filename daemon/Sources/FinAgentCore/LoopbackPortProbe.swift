...
/// Is something actually accepting on a loopback port right now? Shared by VNC (RFB on
/// 5900, docs/VNC.md) and Remote Browser (Chrome's DevTools port, docs/REMOTE-BROWSER.md).
///
/// For VNC specifically:
///
...
public enum LoopbackPortProbe {
    /// macOS Screen Sharing's RFB port — the VNC default. Fixed and well-known; there is no per-machine
    ...
    public static func isReachable(
        host: String = "127.0.0.1", port: UInt16 = LoopbackPortProbe.vncPort, timeout: TimeInterval = LoopbackPortProbe.timeout
    ) -> Bool {
