...
sources:
  - "O007: 1,659 of 2,363 labels are router_baseline / policy_baseline output (gen_training_data.py:276-283, :540)"
...
| --- | --- | --- |
| **A — distillation dominates** | falls well below 24/25 | the fine-tune taught the model to be the baseline. The corpus is the problem, not the recipe. |
| **B — format-only transfer** | holds near 24/25 | the base's own reasoning survives; the fine-tune moved output format and easy-case reliability without displacing judgment. The corpus is harmless but weak. |
...
Arguments for A: 4,490 gradient steps, loss to zero, 890 examples of one
routing policy, LoRA at rank 8 over 16 layers touching 6.9M parameters. That is
enough capacity to shift routing behaviour.

Arguments for B: only **1.69% of the corpus by characters** is unmasked — the
labels — and the system prompt (83.7%) is masked and identical to the one under
which the base already performs at 24/25. The examples the baseline gets right
