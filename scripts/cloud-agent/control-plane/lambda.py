...
# Terminal relay (see the WebSocket route handlers below): fin-terminal-
# connections is keyed by connectionId (so $disconnect, which gets nothing
# else, can find who it was); fin-terminal-sessions is keyed by the app-minted
...
#
# A body dials out only (docs/SITES.md's whole design) — there is no inbound
# path to a site, and a resident Mac behind a firewall/NAT is as reachable as
# an EC2 instance and no more. The 20s site heartbeat above is the liveness/
# command channel, but it is far too slow for an interactive terminal, so this
# is a second, WebSocket-based channel used ONLY while someone actually has a
# terminal open: the app posts a "terminal-open" site command (queued exactly
# like restart/update/stop/drain above); on its next heartbeat the site dials
# this WebSocket API and identifies itself for that one sessionId. Once both
# sides are attached, this Lambda is a pure router — no PTY or tmux logic
# lives here, it only moves JSON frames between two connectionIds by sessionId.
#
# Wire protocol (one JSON object per WebSocket message):
#   app -> relay   {"action": "open",   "sessionId": ..., "siteId": ..., "tmuxSession": ...}
#   site -> relay  {"action": "attach", "sessionId": ...}
#   app -> relay   {"action": "input",  "sessionId": ..., "data": "<base64 bytes>"}
#   app -> relay   {"action": "resize", "sessionId": ..., "cols": N, "rows": N}
#   site -> relay  {"action": "output", "sessionId": ..., "data": "<base64 bytes>"}
#   either -> relay {"action": "close", "sessionId": ..., "reason": "..."}
# "input"/"resize" are forwarded verbatim app -> site; "output" is forwarded
...
def _ws_connection_id(event):
    connection_id = str((event.get("requestContext") or {}).get("connectionId") or "")
    if not connection_id:
        raise ApiError(400, "missing connectionId")
    return connection_id


def _ws_management_client(event):
    ctx = event.get("requestContext") or {}
    domain = ctx.get("domainName")
    stage = ctx.get("stage")
    if not domain or not stage:
        raise ApiError(500, "no management endpoint on this event")
    return _SESSION.client(
        "apigatewaymanagementapi", endpoint_url="https://{}/{}".format(domain, stage)
    )


def _ws_post(client, connection_id, payload):
    """Best-effort send; a gone connection is reported to the caller (who
    cleans up the row) rather than raised, since the caller is mid-relay for
    someone else's frame and must not 500 just because the *other* side hung
    up first."""
    try:
...
def _read_terminal_connection(connection_id):
    return TERMINAL_CONNECTIONS_TABLE.get_item(Key={"connectionId": connection_id}).get("Item")


def _read_terminal_session(session_id):
    return TERMINAL_SESSIONS_TABLE.get_item(Key={"sessionId": session_id}).get("Item")


def _delete_terminal_session(session_id):
...
def ws_connect(event):
    ...
    item = {
        ...
        "connectedAt": _iso(_now()),
        "ttl": int(_now().timestamp()) + TERMINAL_CONNECTION_TTL_SECONDS,
    }
    if role == "site":
        item["siteId"] = event["_siteId"]
    TERMINAL_CONNECTIONS_TABLE.put_item(Item=item)
    return _response(200, {})


def ws_disconnect(event):
    """$disconnect — the socket is already gone; this only cleans up rows.
    Best-effort notifies the other side of the session, if any, so it can
    stop waiting instead of running out the idle timeout."""
    connection_id = _ws_connection_id(event)
    connection = _read_terminal_connection(connection_id)
    TERMINAL_CONNECTIONS_TABLE.delete_item(Key={"connectionId": connection_id})
    session_id = (connection or {}).get("sessionId")
    if not session_id:
        return _response(200, {})
    session = _read_terminal_session(session_id)
    if not session:
        return _response(200, {})
    other_id = None
    if session.get("appConnectionId") == connection_id:
        other_id = session.get("daemonConnectionId")
    elif session.get("daemonConnectionId") == connection_id:
        other_id = session.get("appConnectionId")
    if other_id:
        try:
            client = _ws_management_client(event)
            _ws_post(client, other_id, {"action": "close", "sessionId": session_id, "reason": "peer disconnected"})
        except ApiError:
            pass
    _delete_terminal_session(session_id)
    return _response(200, {})


def _ws_open(event, connection, connection_id, body):
    if connection["role"] != "app":
        raise ApiError(403, "only the app side may open a session")
    session_id = str(body.get("sessionId") or "").strip()
    site_id = str(body.get("siteId") or "").strip()
    if not TERMINAL_SESSION_ID_RE.match(session_id):
        raise ApiError(400, "invalid sessionId")
    site = _read_site(site_id)
    if not site or site.get("userId") != connection["userId"] or site.get("state") == "retired":
        raise ApiError(404, "no such site")
    TERMINAL_SESSIONS_TABLE.put_item(
        Item={
            "sessionId": session_id,
            "siteId": site_id,
            "userId": connection["userId"],
            "tmuxSession": str(body.get("tmuxSession") or ""),
            "appConnectionId": connection_id,
            "createdAt": _iso(_now()),
            "ttl": int(_now().timestamp()) + TERMINAL_SESSION_TTL_SECONDS,
        }
    )
    TERMINAL_CONNECTIONS_TABLE.update_item(
...
def _ws_attach(event, connection, connection_id, body):
    if connection["role"] != "site":
        raise ApiError(403, "only a site may attach")
    session_id = str(body.get("sessionId") or "").strip()
    session = _read_terminal_session(session_id)
    if not session or session.get("siteId") != connection.get("siteId"):
        raise ApiError(404, "no such session for this site")
    TERMINAL_SESSIONS_TABLE.update_item(
        Key={"sessionId": session_id},
        UpdateExpression="SET daemonConnectionId = :c",
    ...
    client = _ws_management_client(event)
    _ws_post(client, session["appConnectionId"], {"action": "attached", "sessionId": session_id})
    return _response(200, {})


def _ws_forward(event, connection, body, *, from_role, to_field):
    """Relays "input"/"resize" (app -> site) and "output" (site -> app)
    verbatim by sessionId, refusing to forward on behalf of a connection that
    isn't actually a party to that session."""
    if connection["role"] != from_role:
        raise ApiError(403, "wrong side for this action")
    session_id = str(body.get("sessionId") or "").strip()
    session = _read_terminal_session(session_id)
    if not session:
        raise ApiError(404, "no such session")
    own_field = "appConnectionId" if from_role == "app" else "daemonConnectionId"
    if session.get(own_field) != connection["connectionId"]:
        raise ApiError(403, "not a party to this session")
    target_id = session.get(to_field)
    if not target_id:
        # The other side hasn't attached yet (e.g. the app is still waking the
        # site) — drop the frame rather than error; the caller keeps retrying
        # input naturally by the user continuing to type.
        return _response(202, {"pending": True})
    client = _ws_management_client(event)
    if not _ws_post(client, target_id, body):
        clear_field = to_field
        TERMINAL_SESSIONS_TABLE.update_item(
            Key={"sessionId": session_id},
            UpdateExpression="REMOVE {}".format(clear_field),
        )
    return _response(200, {})


def _ws_close(event, connection, body):
    session_id = str(body.get("sessionId") or "").strip()
    session = _read_terminal_session(session_id)
    if not session:
        return _response(200, {})
    other_id = None
    if connection["connectionId"] == session.get("appConnectionId"):
        other_id = session.get("daemonConnectionId")
    elif connection["connectionId"] == session.get("daemonConnectionId"):
        other_id = session.get("appConnectionId")
    if other_id:
        client = _ws_management_client(event)
        _ws_post(client, other_id, {"action": "close", "sessionId": session_id, "reason": body.get("reason") or ""})
    _delete_terminal_session(session_id)
    return _response(200, {})


def ws_message(event):
    """$default — every non-connect/disconnect frame, routed by `action`."""
    connection_id = _ws_connection_id(event)
    connection = _read_terminal_connection(connection_id)
    if not connection:
        raise ApiError(401, "unknown connection")
    body = _body(event)
    action = str(body.get("action") or "").strip()
    connection["connectionId"] = connection_id
    if action == "open":
        return _ws_open(event, connection, connection_id, body)
    if action == "attach":
        return _ws_attach(event, connection, connection_id, body)
    if action in ("input", "resize"):
        return _ws_forward(event, connection, body, from_role="app", to_field="daemonConnectionId")
    if action == "output":
        return _ws_forward(event, connection, body, from_role="site", to_field="appConnectionId")
    if action == "close":
        return _ws_close(event, connection, body)
    raise ApiError(400, "unknown action {!r}".format(action))


def _is_websocket_event(event):
    ctx = event.get("requestContext") or {}
    return "connectionId" in ctx and "eventType" in ctx
...
def ws_handler(event):
    ...
    try:
        if event_type == "CONNECT":
            return ws_connect(event)
        if event_type == "DISCONNECT":
            return ws_disconnect(event)
        return ws_message(event)
    except ApiError as exc:
        return _response(exc.status, {"error": exc.message})
    except ClientError as exc:
        LOG.exception("terminal relay: aws call failed")
        return _response(502, {"error": _scrub(exc.response.get("Error", {}).get("Message", "AWS call failed"))})
    except Exception as exc:  # noqa: BLE001 - must not leak a stack trace
        LOG.exception("terminal relay: unhandled error")
...
def lambda_handler(event, _context=None):
    ...
    # The terminal-relay WebSocket API posts a different event shape entirely
    # (requestContext.connectionId/eventType, no requestContext.http, no
    # rawPath) — routed to its own handler before any HTTP-API parsing runs.
    if _is_websocket_event(event):
        return ws_handler(event)
