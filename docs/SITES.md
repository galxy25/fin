...
**The private tmux socket, and what still guards the edges.** `connectCommand` now carries `exec tmux -L fin …`, so the daemon's shell talks to a tmux server that is a different process, with a different socket file, from the one hosting Levi's sessions — and `exec` means leaving that tmux ends the SSH session instead of dropping the agent into the login shell underneath, where `$TMUX` is empty. That is the boundary, and it is topological: no string the model types inside that shell reaches `main`, because the server it would have to talk to is not the one its `$TMUX` points at. It replaces the previous posture — one shared socket plus a 1,579-line parser classifying every `send_input` string — which eight independent reviews walked through **thirty-five distinct high-severity bypasses**, each fix round producing new ones. The cost is the read half, and that is restored structurally rather than given up: the new `read_session` tool takes a session **name** (`^[A-Za-z0-9_.:-]{1,64}$`, no leading `-`) and the daemon runs a fixed `tmux capture-pane -p -J -t <name> -S -<lines>` against the DEFAULT socket over a separate SSH exec channel — the model supplies one word, the daemon supplies every other byte, and the output is capped and redacted. `TmuxCommandGuard` kept only 782 lines of code (down from 1,057) and traded a 90-command policy table for seven rules: refuse EVERY tmux command unless the LIVE shell confirms it is inside Fin's own tmux server (it is asked for `$TMUX` before every tmux-bearing send, not once at launch — the model may leave its own server with `tmux detach`, and a stale "confined" would then let bare tmux commands land on the default socket), refuse a tmux invocation naming another socket (`-L`/`-S`, with `-S` beating `-L` the way tmux itself resolves them), refuse a socket-less tmux that will not run in Fin's own shell (`ssh … tmux …`, `sudo tmux …`, `env -i … tmux …`, `launchctl … tmux …` — `$TMUX` does not survive those, so tmux falls back to the default socket), refuse `kill-server` and its prefixes, refuse any signal aimed at tmux (`pkill tmux`, `kill $(pgrep tmux)` — a signal crosses every socket), refuse any command that unsets or overrides `TMUX` or `TMUX_TMPDIR`, and refuse a half-typed line that mentions tmux (the PTY joins consecutive sends). Everything else on Fin's own server is ordinary work — no allow-list, nothing to register. **Residual, plainly:** tmux reads its socket path from `$TMUX` when no `-L`/`-S` is given, and resolves `-L <label>` under `$TMUX_TMPDIR` (both verified on 3.6a against private sockets only), so an *assembled* edit — a variable set in one send and used in the next, a helper script, an argv built inside an interpreter — still reaches the default socket; the list of environment-crossing wrappers is named rather than fail-closed (the fail-closed version refuses `man tmux`), so an unlisted launcher is a route; the shell runs as Levi's own uid, so that socket is reachable by any process that opens it deliberately — including by replacing Fin's own socket file with a symlink to it; and non-tmux damage was never in scope. The airtight version is a **dedicated UNIX user**, whose different uid cannot open a `0700` socket directory by kernel permission — one `sudo` step, currently out of scope because the installer is deliberately zero-sudo, and Levi's call to make. See `daemon/README.md` § "The tmux boundary" for the rule table and the full residual list.
