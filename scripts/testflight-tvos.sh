...
# App Store Connect requires each upload's CFBundleVersion to be higher than the
# previous one for the same version train. Rather than hand-bumping, ask ASC for
# the highest build number currently on the app record (across all platforms, so
# the number is unambiguous) and use max+1. Falls back to a unix timestamp (always
# increasing) if the API can't be reached. Pass BUILD_NUMBER=N to override.
# CFBundleVersion resolves this via $(CURRENT_PROJECT_VERSION) — see project.yml.
